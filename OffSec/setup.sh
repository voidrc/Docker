...
### CHAL 01 — navigation
mkdir -p /home/ctf/-
echo "school-21{n4v1g4t3_d33p3r_7h4n_y0u_th1nk}" > /home/ctf/-/flag.txt

### CHAL 02 — hidden files
echo "5CH00L-21{h1dd3n_1n_pl41n_s1gh7}" > /home/ctf/.flag.txt
...
chown root:secret /etc/s21_groups
chmod 750 /etc/s21_groups
chown root:secret /etc/s21_groups/flag.txt
chgrp secret /etc/s21_groups/flag.txt
chmod 440 /etc/s21_groups/flag.txt

### CHAL 04 — sudo (limited)
echo "ctf ALL=(root) NOPASSWD:/bin/cat,/bin/ls" > /etc/sudoers.d/ctf
...
pkill -f "nc -l -p 9001" 2>/dev/null || true
nohup sh -c "while true; do nc -l -p 9001 < /etc/service_flag.txt; done" >/dev/null 2>&1 &
